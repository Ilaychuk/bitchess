...
extern const bitboard_t files[8] = {
	...
	[GFILE] = 0x4040404040404040,
	[FFILE] = 0x2020202020202020,
	[EFILE] = 0x1010101010101010,
	[DFILE] = 0x0808080808080808,
	[CFILE] = 0x0404040404040404,
...
bool boundry_check(bitboard_t bitboard, direction_e dir)
{
	switch (dir) {
		...
		case WEST:
			bitboard &= files[HFILE];
			break;

		case EAST:
			bitboard &= files[AFILE];
			break;
...
permutation_t get_occupancy_permutation(size_t idx, bitboard_t mask)
{
	permutation_t p = 0;
	...
	for (size_t i = 0; i < set_bits_size; i++) {
		...
		size_t bit_idx = get_first_bit(mask);
